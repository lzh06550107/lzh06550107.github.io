...
**日期：** 2026-06-09
**状态：** Analysis / triage. We are implementing these ourselves; the referenced
community PRs are evidence and 引用 material, **not** code we intend to merge.

## Purpose

A single place that captures every open issue and PR touching the visual
brainstorming companion (the 本地 服务器 in `skills/brainstorming/scripts/`),
distilled to the underlying problem and the change we'd make. Each item is
grounded against the 当前 code, not the PR author's description.
...
- **E1 (terminal-vs-HTML hard gate) is a workshop item.** We'll 设计 it
...
- **Remote serving is a first-class scenario.** Superpowers is general-purpose;
  ...
  安全 fix MUST protect those users, not just loopback. **Decision: a
  per-session secret key**, not a Host allowlist. A Host allowlist only
  defends the loopback browser-confused-deputy; a direct remote 客户端 just
  sends the expected `Host`, so the allowlist is theater for remote exposure. A
  secret key is the only thing that authenticates a 客户端 uniformly across
...
| File | Role |
|------|------|
| `skills/brainstorming/scripts/server.cjs` | Zero-dep HTTP + WebSocket 服务器 (RFC 6455 hand-rolled). Serves the newest screen, watches `content/`, records events to `state/events`. |
| `skills/brainstorming/scripts/helper.js` | Injected into every page. WebSocket 客户端, click capture, `window.brainstorm` API. |
...
| `skills/brainstorming/scripts/start-server.sh` | Launch wrapper. Session dir, host/url-host, owner-PID resolution, 平台 backgrounding. |
| `skills/brainstorming/scripts/stop-server.sh` | Kills the 服务器 by PID file, cleans `/tmp` sessions. |
| `skills/brainstorming/visual-companion.md` | Operator guide the agent 读取 when it accepts the companion. |
...
| B3 | WS 客户端 reconnect backoff + status indicator | PR #856 | Do |
...
| E4 | Shell-lint 警告 in start/stop scripts | PR #1677 | Opportunistic only |

---

## A. Server 安全 加固 (`server.cjs`)

### A1 — Per-session secret key (chosen approach)

**Threat model.** Two assets: confidentiality of the served screen (`/`) and
files (`/files/*`), and integrity of `state/events` — a WebSocket 客户端 with a
truthy `choice` writes there (`server.cjs:243-246`), and the agent 读取 it next
turn as the user's selection, i.e. **提示词 injection into a live session with
...
loopback browser-deputy. A direct remote 客户端 just sends the expected `Host`
and forges/omits `Origin`, so the allowlist is theater for exactly the remote
case we must protect. A per-session secret authenticates the 客户端 uniformly
...
1. **URL carries it** as `?key=<token>`. The 服务器 already builds `url` in its
...
3. **Auth gate** = valid `?key` **OR** valid cookie (compared with
   ...
   key → 友好 **403 HTML page** ("this page needs the full URL your coding
...
The query token is the 事实来源; the cookie is a convenience that never
bears initial-auth load.

**Blast radius.** `server.cjs` (all logic). `helper.js` 可选 one-liner
(append `?key=` from `location.search` to the WS URL as a cookie-blocked
回退). `start-server.sh` none. `visual-companion.md` doc note (URL now has
...
实施 still checks browser WebSocket `Origin` after session auth so a
...
`if (event.choice)` at `server.cjs:243`. A 客户端 that sends the 4-byte text
...
result is an uncaught exception and process exit. Any 本地 客户端 can kill the
server.
...
Referenced by PR #1504 as #1446. The 当前 code **already** bounds extended
...
当前 `dev` and close if already resolved, rather than re-implementing.
...
**Problem.** `stop-server.sh` 读取 the PID from `state/server.pid`
...
running our `server.cjs`, ideally matching this session. 如果 ownership can't be
proven, fail closed (报告 `stale_pid`, don't kill). Keep the 现有
`stopped` / `not_running` outputs for the real cases.

### B3 — WebSocket 客户端: silent reconnect, stale "Connected"
...
dot pinned to `var(--success)` (`frame-template.html:77,200`). 当 the laptop
sleeps or the 服务器 restarts, the page shows "Connected" over a dead socket and
...
服务器 dies mid-session. Separately, `shutdown()` (`server.cjs:310-321`) calls
...
**Problem.** 当 the 服务器 exits it writes `state/server-stopped` and removes
...
- **Browser-facing tombstone.** Leave something at the last-served URL that says
  "this companion expired — ask Claude to 重启 it" instead of a connection
  error. Options to weigh: `helper.js` rendering a banner when the socket stays
  down past backoff (works only while the page is 已加载), vs. a more involved
...
- **Harder skill check.** Tighten `visual-companion.md` / `SKILL.md` so
  ...
  screen" is a 必需 step, not a note. Keep it lightweight — possibly a
...
**Problem.** The 客户端 only captures clicks on `[data-choice]`
...
**Change.** 添加 a feedback `<textarea>` whose submit emits
`{"type":"feedback","text":...,"timestamp":...}` via the 现有
`window.brainstorm.send` path (`helper.js:82-85`).

**Cross-cutting — 服务器 change required.** `handleMessage` only persists events
...
`feedback` events. Document the 新 event shape in `visual-companion.md`
...
失败, never block startup, keep echoing the URL. Document in
`visual-companion.md`. (Consider an opt-out for headless/remote runs where
popping a browser is wrong — ties into D1's 配置 mechanism.)
...
**Change.** 添加 `.light-surface` / `.dark-surface` helper classes plus a
conservative 回退 for common inline light backgrounds, and document them in
...
vendored third-party 依赖 in the companion runtime. The underlying need
(interactive mockups) is not being pursued via this route.

### E4 — Shell-lint 警告 (PR #1677) — OPPORTUNISTIC
...
E1 is a separate workshop session. E2/E3 are 范围外 for this round.
