...
The 当前 file delegates to `../requesting-code-review/code-reviewer.md`, which is a merge-readiness 审查 (架构, 安全, production readiness, "Ready to merge?"). 替换 the entire file with a self-contained, task-scoped template.
...
Subagent (general-purpose):
  ...
  提示词: |
    You are reviewing one 任务's 实施 for code quality. This is a
    task-scoped gate, not a merge 审查 — a broad whole-branch 审查 happens
    separately after all 任务 are complete.
    ...
    **Head:** [HEAD_SHA]
    ...
    Your 审查 is read-only on this checkout. Do not mutate the working tree,
    the index, HEAD, or branch state in any way. 使用 tools like `git show`,
    `git diff`, and `git log` to inspect history.

    ## Scope

    Spec compliance was already verified by a separate reviewer. Do not
    re-check whether the code matches the 需求 or the plan.
    ...
    report. 运行 a test only when reading the code raises a specific doubt
    that no 现有 run answers — and then a focused test, never a
    package-wide suite, race detector run, or repeated/high-count loop. 如果
    heavy validation seems warranted, recommend it in your 报告 instead of
    running it. 如果 you cannot run commands in this environment, name the
    test you would run.
    ...
    **Tests:**
    - Do the 新 and changed tests verify real behavior, not mocks?
    - Are the 任务's edge cases covered?

    **Structure:**
    - Does each file have one clear responsibility with a well-defined interface?
    - Are units decomposed so they can be understood and tested independently?
    - Is the 实施 following the 文件结构 from the 计划?
    - Did this change create 新 files that are already large, or
      significantly grow 现有 files? (Don't flag pre-existing file
    ...
    [Poor 错误 handling, test gaps, structural problems]
...
- [ ] **步骤 2： Trim the read-only section.** 替换 (currently line 35):
...
- [ ] **步骤 3： Ground the skepticism.** 替换 (currently lines 39-40):
...
- [ ] **步骤 4： 添加 the third verdict channel.** 替换 (currently lines 74-76):
...
### Task 3: Implementer 提示词 — re-run tests after fixing 审查 findings
...
- [ ] **步骤 1： Point the final-review flowchart node at the broad template.** The node label `Dispatch final code reviewer subagent for entire implementation` appears 3 times (currently lines 65, 84, 85). In all 3 occurrences, replace the label string with:
...
- [ ] **步骤 3： 添加 controller guidance sections.** Immediately before the line (currently line 122):
...
- [ ] **步骤 4： Prompt Templates list — add the final-review pointer.** 替换 (currently line 126):
...
- [ ] **步骤 5： Example 工作流 verdict vocabulary.** Two replacements:
...
(The final 审查者's "ready to merge" line, currently line 199, stays.)
...
### Task 5: New eval 场景 — per-task quality 审查者 catches a planted defect
...
- [ ] **步骤 2： Register the helper.** In `evals/setup_helpers/__init__.py`:
...
- [ ] **步骤 5： 创建 `evals/scenarios/sdd-quality-reviewer-catches-planted-defect/checks.sh`** (no executable bit):
...
预期: all pass; `quorum check` lists the 新 场景 without errors.
...
### Task 6: Static 验证 sweep

**文件:** none modified — 验证 only.
...
- [ ] **步骤 1： Baseline (skills as released on dev)** — from the main checkout (`/Users/jesse/git/superpowers/superpowers`, on dev), or any checkout without this branch's changes:
...
- [ ] **步骤 2： After (this branch's skills)** — point `SUPERPOWERS_ROOT` at this worktree:
