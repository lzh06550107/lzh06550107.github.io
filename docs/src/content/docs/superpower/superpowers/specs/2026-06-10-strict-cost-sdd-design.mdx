...
当前 config. Any quality regression kills the rung, full stop.

## Where the dollars are (final 2026-06-10 配置, go-fractals, ~$13/run)
...
| Final 审查 + fixes | ~1 | 6 turns with branch package |

Review-loop count (2-4 per run) is the biggest run-to-run 成本 variance;
loops are mostly caused by 计划 ambiguity the implementer resolved wrongly.
...
- **BLOCKED / NEEDS_CONTEXT handling** — diagnosing why a 子 agent is stuck
...
- **Escalate-to-human recognition** — knowing the 计划 itself is wrong

A rung that would move any of these to a cheaper model must either (a)
restructure so the decision is made once by the expensive model at 计划
...
acceptance evidence — judgment 失败 are rare-event, high-blast-radius,
...
baseline) in addition to the N=5 场景 gates.

## Thesis guardrail

SDD's thesis: **a 全新 子 agent per 任务 with precisely curated context,
gated per task.** Rungs below must preserve it. Dispatch-time 任务 batching
(one implementer 派发 handling several 计划 任务) is **counter-thesis**
...
派发 economics is plan-time 任务 right-sizing (L1): if the 计划 defines
fewer, better-sized 任务, SDD still runs one 全新 子 agent per task.

## The ladder (in expected $/leverage order)

### L1 — Plan-side crispness (writing-plans changes; est. −$1.5-3/run, plus variance reduction)

**状态 2026-06-11 (final): elicitation tested end-to-end; claims
re-attributed.** Micro-tests: 约束 header and Interfaces blocks
elicit deterministically (0→5/5, 0→100% of 任务, exact values);
right-sizing is modest and scale-dependent (9.4→8.4 任务 at svelte
scale, nothing to move at fractals scale). Full runs: an elicited 计划
executed at $6.34/$8.49 — but the no-guidance control (opus 计划,
complete code) hit $7.59/$7.73, inside that range. **The 成本 win
...
deterministic 约束 propagation (the one elicited-run fix was a
version-floor catch), exact cross-task interfaces, fix waves 1 vs 2-4
(the control 计划 shipped a real Sierpinski bug both runs had to fix).
The writing-plans PR claims those grounds, not dollars. Draft at
/tmp/sdd-exp/writing-plans-l1 (branch writing-plans-crisp).

The 计划 is upstream of every 成本: 任务 count sets 派发 count; 计划
ambiguity sets review-loop count; 计划 completeness sets implementer
exploration. Current writing-plans optimizes for implementer 成功, not
execution economics. Changes to test:

1. **Task right-sizing guidance.** Today's plans produce 任务 as small as
   "create .gitignore" — each costing a full 派发 + 审查 cycle
   (~$0.60-1.00 fixed overhead). 添加: "A 任务 is the smallest unit that
   carries its own test cycle and is worth a 全新 审查者's gate. Merge
   setup/config steps into the 任务 that needs them; split only at
   boundaries where a 审查者 could meaningfully reject." Fractals' 计划
   would drop from 10 任务 to ~7. Validate: 派发 count falls, gates
   hold, 审查 granularity still catches the planted defect.
2. **Structured `## Global Constraints` section** in the 计划 header
   (version floors, naming/copy rules, 平台 需求). Today these
   ...
   `task-brief` can append them to every brief 自动 (small script
   change), removing a controller responsibility entirely.
3. **Per-task `Interfaces:` line** (consumes/produces, exact signatures).
   The controller currently re-derives cross-task interfaces per 派发
   ...
   line per 任务 moves the work to where it is done once.
4. **Per-task model-tier recommendation** from the planner ("mechanical /
   ...
   Model Selection decision the controller currently re-makes per 派发;
...
计划 *placeholders* cannot be elicited from 当前 opus — these changes
...
**状态 2026-06-11 (final): DIED AT THE GATES, as pre-registered — with
useful anatomy.** Recon was positive ($6.68/$8.05, n=2, mechanics clean).
The full battery split the judgment surface: the 新
`sdd-escalates-broken-plan` 场景 (explicit 计划 self-contradiction;
...
assertion, as 必需" listed under Strengths), the defect shipped in
4/5 runs (deterministic check), and only the tier-pinned opus final
审查者 ever caught it — while the same sonnet-tier reviewers under an
...
A possible L2b (discrete rule: "a 审查者 finding that conflicts with
the 计划's text is the human's decision — escalate it") would route the
...
opus stack, does NOT rescue the sonnet rung.** Two rules: a 审查者
...
(plan-mandated findings go to the human like any 计划 contradiction).
...
审查者's miss as self-described backstop, and escalated for a
...
alone doesn't fire it live (read-once dilution across the 审查者's
tool 读取; placement within the 派发 refuted as the variable),
...
opus controllers showed; 审查 loops, per-task Important→fix→re-review,
...
1. **Primary form — judgment moved up front, mechanics cheapened:** the
   expensive model does the judgment-dense work at 计划 time (L1's
   Interfaces lines, ambiguity resolutions, per-task 约束 — i.e.
   the 派发 curation is pre-written into the 计划). The mid-tier
   execution session then runs a loop that is genuinely mechanical:
   extract brief, 派发, run script, route verdicts. Explicit
   escalation rules in the skill: on BLOCKED, on any ⚠️ item, on a
   suspected false positive, or on anything the 计划 does not already
   answer, the cheap controller STOPS and escalates (to the human, or to
   a 全新 expensive-model consultation 派发) — it never resolves
   judgment alone.
2. **Gates beyond the standard N=5:** a judgment audit — every
   ...
   rung regardless of 场景 verdicts.
...
multi-step *work*, not 派发 loops; whether a mid-tier controller holds
...
**状态 2026-06-11: DEAD, as pre-registered.** Planted-defect ×5 with
forced-haiku 任务 reviewers: 2 pass / 1 indeterminate / 2 fail (baseline
...
The package 审查者 is near-single-step mechanically (3 turns / 1 Read
...
- `task-brief --list` mode: controller 读取 任务 headings + Global
  Constraints, never the full 计划 (the 计划 body is already delivered via
  briefs).
...
- **Dispatch-time 任务 batching**: counter-thesis (see guardrail). L1.1
  is the sanctioned form.

## Budget and sequencing

L1 and L2.1 are independent — run both first (~$80: micro-tests + 2×5-run
gates + A/B). L3 after L2 settles the controller (审查者 behavior depends
on 派发 quality; ~$25 — planted-defect runs are $2-3 each). L4 last
(cheap, but re-gate once after the stack; ~$30). Total ≲ $150 for the full
ladder with honest N=5 gates. 预期 end state if every rung survives its gates: **$5-7/run on
...
## Relationship to 现有 work

Builds on the 2026-06-09 task-scoped 审查 派发 设计 (PR #1717) and
...
parallel-call mechanisms are dead). Instruction wording for any 新 prose
