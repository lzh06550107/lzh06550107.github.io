---
title: "Skill 指导的正向指令重设计 —— 设计规格"
description: "由 Markdown 原样翻译并转换为 Astro Starlight MDX 格式。"
---

# Skill 指导的正向指令重设计 —— 设计规格

**状态：** Proposed (follow-up to the 2026-06-09 SDD review-dispatch work; separate PR per the one-problem-per-PR rule)
...
| Dispatch composition | positive recipe ("your 派发 should contain: (1)…(5)") | **3.0, zero variance** — adopted |
| Dispatch composition | recipe + nuance clause ("quote only the fragment…") | 3.8, noisy — nuance dilutes recipes |
| Test-rerun directive ("do not ask 审查者 to re-run tests") | prohibition | **0/5 violations** — works fine (control: 3/5) |
...
1. **Tripwires work.** Phrase-level self-checks on concrete tokens ("if the
   提示词 you are writing contains 'do not flag' … 停止") fire reliably.
...
## Audit results (2026-06-10, all ~30 skills + 提示词 templates)
...
|---|---|---|
| 1 | `subagent-driven-development/task-reviewer-prompt.md` — "Cite, don't narrate" | **Queued in PR #1717 batch**: lead with the positive half ("Your 报告 should point at evidence: file:line for every finding…"), drop the prohibition half (dead weight — the positive half already exists and carries the load) |
| 2 | `subagent-driven-development/SKILL.md` — "Do not add open-ended directives" | **Keep as-is**: micro-test could not elicit the 失败 in 15 samples; no evidence either way; shorter wins |
| 3 | `subagent-driven-development/SKILL.md` — "Do not ask a 审查者 to re-run tests" | **Keep as-is**: measured 0/5 violations; the prohibition also usefully propagates itself into dispatches |
| 4 | `subagent-driven-development/SKILL.md` — "do not re-review on top of it" | **Queued in PR #1717 batch**: replace with the three-element checklist ("Before re-dispatching the 审查者, confirm the fix 报告 contains: the covering tests, the command run, and the output") |
...
'添加 appropriate 错误 handling', 'Write tests for the above', 'Similar to
Task N', …").

### Why it matters and why it is genuinely uncertain

- Plans are the **largest generated artifact** in the 工作流, and the
...
- **The list is load-bearing elsewhere:** the skill's Self-Review section
  引用 it ("Placeholder scan: search your 计划 for red flags — any
  ...
  that 引用 and discards good tripwire tokens.

### Variants to test

- **V0 (当前):** positive sentence + banned list at composition time;
  Self-Review 引用 the list.
- **V1 (auditor's checklist):** composition-time positive recipe only —
  ...
  within this 计划, 错误 handling shown explicitly. A step is complete
...
- **Task:** opus writes a 2-3 任务 实施计划 from a deliberately
  under-specified spec (under-specification is what tempts placeholders).
  使用 a fixture spec with: one well-specified 任务, one 任务 whose 错误
  handling the spec hand-waves, one 任务 similar to the first (tempting
  "Similar to Task 1").
...
- **Programmatic scoring** (lower is better unless noted):
  ...
  - 引用 to types/functions not defined anywhere in the 计划 output
  - (higher is better) runnable commands with expected output per 任务
- **Two-stage scoring for V2:** also test the Self-Review half — feed each
  generated 计划 back with the variant's Self-Review section and measure
  whether the scan actually catches seeded placeholders (insert 2 known
  placeholders into a fixture 计划; detection rate is the metric).
...
- One API call per sample: system 提示词 = the skill-guidance variant in
  realistic surrounding context; user = a realistic mid-workflow 场景;
  output = the composed artifact (派发 提示词, 计划, 报告).
...
- Always include a no-guidance control — tonight it revealed both a
  ...
  prohibition (test-reruns: 3/5 control 失败 vs 0/5 with either
  phrasing).
...
TBD/TODO ✓". Current-generation opus does not produce 计划 placeholders
...
PR. The V2 relocation 设计 remains on file here should a future model
generation regress.

## Also explicitly not-dropped (tested-and-declined, with data)

Recorded so nobody re-proposes them without 新 evidence — full numbers in
the 2026-06-09 SDD 设计规格's Cost-iterations section:
