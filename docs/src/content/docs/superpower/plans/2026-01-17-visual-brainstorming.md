...
> **对于代理型工作者：** 必需子技能：使用 superpowers:executing-plans 按任务逐步实施此计划。

**目标：** 为 Claude 提供一个基于浏览器的视觉伴侣，用于头脑风暴会话——在终端对话旁边显示模型图、原型和交互式选择。

**架构：** Claude 将 HTML 写入临时文件。本地 Node.js 服务器监听该文件，并通过自动注入的辅助库提供服务。用户交互通过 WebSocket 流向服务器 stdout，Claude 可在后台任务输出中看到这些内容。
...
将 HTML 写入 `/tmp/brainstorm/screen.html`。服务器会监听此文件并自动刷新浏览器。
...
当头脑风暴涉及视觉元素——UI 模型图、线框图、交互式原型——使用基于浏览器的视觉伴侣。
...
终端仍然是主要对话界面。浏览器是视觉辅助工具。

**参考：** 请参阅此技能目录中的 `visual-companion.md`，获取 HTML 模式和 API 细节。
```
...
1. **服务器** 位于 `lib/brainstorm-server/`——监听 HTML 文件并中继事件的 Node.js 服务器
2. **辅助库** 自动注入——捕获点击、表单、输入
3. **测试** 位于 `tests/brainstorm-server/`——验证服务器行为
4. **头脑风暴技能** 已更新，加入视觉伴侣部分和 `visual-companion.md` 参考文档
