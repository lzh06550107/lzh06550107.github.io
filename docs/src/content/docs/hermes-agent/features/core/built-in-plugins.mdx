...
向导将收集您的密钥，通过 `pip` 安装 `langfuse` SDK，并自动将 `observability/langfuse` 添加到 `plugins.enabled` 中。重启 Hermes 后，下一轮对话就会发送追踪数据。

**设置（手动）：**

1. 安装 SDK：`pip install langfuse`
2. 启用插件：`hermes plugins enable observability/langfuse`
3. 在 `~/.hermes/.env` 中配置凭据：
