...
description: hermes agent 持久记忆 (Persistent Memory)
---
...
| 特性 | 持久记忆 (Memory) | 会话搜索 (Session Search) |
| --- | --- | --- |
| **容量** | 总计约 1,300 tokens | 无限制（所有会话） |
| **速度** | 即时（已在系统提示词中） | 需要搜索 + LLM 总结 |
| **用例** | 始终可用的核心事实 | 寻找特定的过往对话 |
| **管理** | 由代理手动策划 | 自动 —— 存储所有会话 |
| **Token 成本** | 每个会话固定（约 1,300 tokens） | 按需消耗（仅在搜索时产生） |

**内存（Memory）** 用于存放那些应始终处于上下文中的关键事实。**会话搜索（Session search）** 则用于诸如 “我们上周讨论过 X 吗？” 之类的查询，即代理需要回忆过去对话中的具体细节时使用。
