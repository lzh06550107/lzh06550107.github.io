...
技能（Skills）是代理在需要时可以加载的按需知识文档。它们遵循**渐进式披露**（progressive disclosure）模式，以最小化 Token 使用量，并兼容 `agentskills.io` 开放标准。

所有技能都存储在 `~/.hermes/skills/` 中——这是主要目录，也是唯一的真理来源。在全新安装时，内置技能会从仓库中复制到此处。通过 Hub 安装以及代理创建的技能也会存放在这里。代理可以修改或删除任何技能。
...
| **skills-sh** | `skills-sh/vercel-labs/...` | 通过 `hermes skills search <query> --source skills-sh` 搜索。当 slug 与仓库文件夹不同时，Hermes 会解析别名式技能。 |
| **well-known** | `well-known:[https://mintlify.com/](https://mintlify.com/)...` | 直接从网站的 `/.well-known/skills/index.json` 提供的技能。使用站点或文档 URL 进行搜索。 |
| **url** | `[https://sharethis.chat/SKILL.md](https://sharethis.chat/SKILL.md)` | 指向单文件 `SKILL.md` 的直接 HTTP(S) URL。名称解析顺序：Frontmatter → URL 后缀 → 交互式提示 → `--name` 标志。 |
...
| **trusted** | 知名注册表（如 `openai/skills`） | 比社区来源更宽松的策略 |
| **community** | 其他所有来源（`skills.sh`, URL 等） | 非危险发现可用 `--force` 覆盖；“危险” 判定保持拦截 |
...
* 目录名称即为技能的安装标识（slug）。
* 每个技能目录必须包含一个 `SKILL.md` 文件，并带有标准的 **SKILL.md frontmatter**（包含 `name`、`description`，以及可选的 `metadata.hermes.tags`、`version`、`author`、`platforms`、`metadata.hermes.config`）。
