...
Hermes 会存储会话历史以便恢复对话，但它不会重复发送处理过的每一个字节。在每一轮对话中，模型会看到选定的系统提示词、当前的对话窗口以及 Hermes 为该轮对话显式注入的任何内容。
...
:::tip[提示]
当会话变长时，使用 `/compress`；如需新线程，使用 `/new`；而 `hermes sessions prune` 仅在你希望从存储中删除已结束的老旧会话时使用。压缩可以减少活跃的上下文，它并不是一种隐私删除。
:::
...
* sort — 可选 `newest`（最新）或 `oldest`（最旧），在 FTS5（全文检索）排序的基础上生效。省略此参数则仅按相关性排序（默认值；适用于探索性召回）。对于 “我们在哪里留下了 X” 这类问题，使用 `newest`；对于 “X 是如何开始的” 这类问题，使用 `oldest`。
* role_filter — 以逗号分隔的需要包含的角色。探索发现默认值为 `user,assistant`（工具输出通常是噪音）。传入 `user,assistant,tool` 可包含工具输出（用于调试工具行为），或传入 `tool` 以仅搜索工具输出。
