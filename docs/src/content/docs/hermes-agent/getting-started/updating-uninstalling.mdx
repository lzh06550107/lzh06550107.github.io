...
1. **配对数据快照 (Pairing-data snapshot)** —— 保存一个轻量级的更新前状态快照（涵盖 `~/.hermes/pairing/`、飞书评论规则以及其他在运行时修改的状态文件）。可以通过 [快照与回滚](https://hermes-agent.nousresearch.com/docs/user-guide/checkpoints-and-rollback) 中描述的流进行恢复，或者解压 Hermes 在 `~/.hermes/` 目录旁生成的最新快速快照 zip 文件。
