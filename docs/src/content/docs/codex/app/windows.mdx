...
在全访问模式下运行 Codex 意味着 Codex 不再受限于你的项目目录，可能会执行意外的破坏性操作，从而导致数据丢失。请保持沙盒边界，使用[规则](/codex/configuration/rules)进行针对性的例外处理，或将你的[审批策略](/codex/configuration/permissions)设置为「从不」，让 Codex 根据你的审批和安全设置在无需请求提权的情况下尝试解决问题。
