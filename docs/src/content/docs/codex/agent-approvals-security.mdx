...
有关更广泛的配置工作流程，请参阅[配置基础](/codex/config-basic)、[高级配置](/codex/config-advanced)和[配置参考](/codex/config-reference)。
...
- 如果你不希望 Codex 将会话记录保存在 `CODEX_HOME` 下，请检查本地数据保留设置（例如 `history.persistence` / `history.max_bytes`）。请参阅[高级配置](/codex/config-advanced)和[配置参考](/codex/config-reference)。
...
企业管理员可以在[托管配置](/codex/enterprise/index)中为其工作区配置 Codex 安全设置。请参阅该页面了解设置和策略详情。
