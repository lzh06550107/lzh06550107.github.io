...
对于 sandbox 和 approval 键（`approval_policy`、`sandbox_mode` 和 `sandbox_workspace_write.*`），请结合 [Sandbox 和审批](/codex/sandbox-and-approvals)、[可写根目录中的受保护路径](/codex/protected-paths) 和 [网络访问](/codex/network-access) 来阅读此参考。对于 beta 权限配置文件，请参阅 [Permissions](/codex/permissions)。
...
| 键 | 类型/值 | 详情 |
|---|---|---|
| `hooks` | table | 在 `config.toml` 中内联配置的生命周期钩子。使用与 `hooks.json` 相同的事件模式；有关示例和支持的事件，请参阅 [Hooks 指南](/codex/hooks)。 |
