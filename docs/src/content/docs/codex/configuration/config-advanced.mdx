...
配置文件（Profiles）允许您保存命名的配置值集合，并从 CLI 切换使用。

> **实验性功能**：配置文件是实验性的，可能在未来的版本中更改或移除。
>
> 配置文件目前不支持 Codex IDE 扩展。
...
要将某个配置文件设为默认，请在 `config.toml` 顶层添加 `profile = "deep-review"`。Codex 会加载该配置文件，除非您在命令行中覆盖它。

配置文件也可以覆盖 `model_catalog_json`。当顶层和所选配置文件都设置了 `model_catalog_json` 时，Codex 优先使用配置文件的值。
...
项目配置文件不能覆盖会重定向凭据、更改提供商身份验证或运行机器本地通知/遥测命令的设置。Codex 会在项目本地的 `.codex/config.toml` 中忽略以下键，并在检测到它们时打印启动警告：`openai_base_url`、`chatgpt_base_url`、`model_provider`、`model_providers`、`notify`、`profile`、`profiles`、`experimental_realtime_ws_base_url` 和 `otel`。请将这些键设置在您的用户级 `~/.codex/config.toml` 中。
...
启用了数据驻留的项目可以创建模型提供商，使用正确的前缀更新 `base_url`。
