...
有关项目引导、可复用能力、自定义斜杠命令、子智能体工作流和集成的背景信息，请参阅[自定义](/codex/getting-started/concepts/customization)。关于配置键，请参阅[配置参考](/codex/config-reference)。
...
有关身份验证的详细信息（包括凭据存储模式），请参阅[身份验证](/codex/authentication)。关于完整的配置键列表，请参阅[配置参考](/codex/config-reference)。
...
关于当前事件列表、输入字段、输出行为和限制，请参阅 [Hooks](/codex/hooks)。
...
有关一起配置文件系统和网络访问的 Beta 权限配置文件，请参阅[权限](/codex/permissions)。
...
关于内置配置文件、自定义配置文件语法以及完整的文件系统和网络配置模型，请参阅[权限](/codex/permissions)。

关于完整的键列表，包括配置文件范围覆盖和要求约束，请参阅[配置参考](/codex/config-reference)和[托管配置](/codex/managed-config)。
...
请参阅专门的 [MCP 文档](/codex/mcp)了解配置详情。
...
如果 `exporter = "none"`，Codex 会记录事件但不发送任何内容。导出器异步批处理，并在关闭时刷新。事件元数据包括服务名称、CLI 版本、环境标签、对话 ID、模型、沙箱/审批设置和每事件字段（参见[配置参考](/codex/config-reference)）。
...
请参阅[配置参考](/codex/config-reference)了解确切的键。
...
有关详细说明，请参阅[使用 AGENTS.md 自定义指令](/codex/agents-md)。
...
请参阅[配置参考](/codex/config-reference)了解完整的键列表。
